...
same ideas inside an 802.11n HT SISO packet-level simulation.
...
| `802.11n_HT_SISO` | Packet-level 802.11n HT 20 MHz SISO simulations for both Full and Lite TScatter. | `Full_TScatter_80211n_SISO.m`, `Lite_TScatter_80211n_SISO.m` |
...
The packet-level scripts in `802.11n_HT_SISO` use MATLAB WLAN Toolbox
components in the style of the MathWorks 802.11n TGn packet-error-rate
example:
...
This repository also includes `CITATION.cff` so GitHub can show a
machine-readable "Cite this repository" entry.
...
再把同样的机制放进 802.11n HT SISO 的 packet-level 仿真链路里验证。
...
| `802.11n_HT_SISO` | Full 和 Lite TScatter 在 802.11n HT 20 MHz SISO 上的 packet-level 仿真。 | `Full_TScatter_80211n_SISO.m`, `Lite_TScatter_80211n_SISO.m` |
...
`802.11n_HT_SISO` 文件夹里的脚本使用 MATLAB WLAN Toolbox 的真实接收流程，
结构参考 MathWorks 的 802.11n TGn packet-error-rate 示例：
...
这个仓库也包含 `CITATION.cff`，这样 GitHub 可以显示机器可读的
“Cite this repository” 信息。
